...
public:
	...
	void Add_AllocationTable(int start_address, int size, int allocation_number);//登记已分配区表
	...
	if (size <= head_free->size)//判断内存空间剩余所有容量是否大于等于申请需求
	{
		...
		while (p_work)
		{
			if (p_work->status == true && p_work->size > size)//如果是空闲分区 && 空间 > 需求量
			{
				...
				p_work->size = p_work->size - p_new->size;//p_work属于空闲区，p_new分配的主存空间是从p_work上分割出来的
